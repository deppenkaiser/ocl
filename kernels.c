...
// iwt_flux bleibt beim Namen, aber rechnet jetzt komplex
protected const char* ocl_get_source_iwt_flux(void)
{
    ...
    "    double phase_i = I_phase[idx];\n"
    "    double real_i = abs_i * cos(phase_i);\n"
    "    double imag_i = abs_i * sin(phase_i);\n"
    "\n"
    "    double sum_real = 0.0;\n"
    "    double sum_imag = 0.0;\n"
    "\n"
    ...
    "        double phase_j = I_phase[j];\n"
    "        double real_j = abs_j * cos(phase_j);\n"
    "        double imag_j = abs_j * sin(phase_j);\n"
    "\n"
    "        double diff_real = real_i - real_j;\n"
    "        double diff_imag = imag_i - imag_j;\n"
    "\n"
    "        double Kij = K[idx * N + j];\n"
    "        sum_real += Kij * diff_real;\n"
    "        sum_imag += Kij * diff_imag;\n"
    "    }\n"
    "\n"
    "    sumJ[idx * 2 + 0] = sum_real;\n"
    "    sumJ[idx * 2 + 1] = sum_imag;\n"
    "}\n";
}

// iwt_q bleibt beim Namen, rechnet aber aus der Amplitude I_abs
...
protected const char* ocl_get_source_iwt_update_info(void)
{
    ...
    "\n"
    "    // ============================================\n"
    "    // FUNDAMENTALE DYNAMIK: DER FLUSS\n"
    "    // NICHT die Wellengleichung!\n"
    "    // ============================================\n"
    "\n"
    "    int x = i % width;\n"
    "    int y = i / width;\n"
    "    int is_boundary = (x == 0 || x == width - 1 || y == 0 || y == width - 1);\n"
    "\n"
    "    // 1. Komplexen Wert aus Amplitude und Phase rekonstruieren\n"
    "    double real_i = I_abs[i] * cos(I_phase[i]);\n"
    "    double imag_i = I_abs[i] * sin(I_phase[i]);\n"
    "\n"
    "    // 2. Fluss anwenden: Knoten i bekommt den Fluss von allen anderen\n"
    "    //    Der Fluss ist bereits in sumJ gespeichert (von i zu allen j)\n"
    "    //    Wir verwenden den FLUSS, nicht die Wellengleichung!\n"
    "    double sumJ_real = sumJ[i * 2 + 0];\n"
    "    double sumJ_imag = sumJ[i * 2 + 1];\n"
    "\n"
    "    // 3. Update der komplexen Amplitude durch den Fluss\n"
    "    //    Die Information bleibt erhalten, weil der Fluss antisymmetrisch ist.\n"
    "    double real_new = real_i + DT * sumJ_real;\n"
    "    double imag_new = imag_i + DT * sumJ_imag;\n"
    "\n"
    "    // 4. Q beeinflusst die Phase (Führung, kein Energietransport)\n"
    "    double phase_new = atan2(imag_new, real_new);\n"
    "    phase_new = phase_new + DT * Q[i];\n"
    "\n"
    "    // 5. Amplitude extrahieren\n"
    "    double abs_new = sqrt(real_new * real_new + imag_new * imag_new);\n"
    "    if (abs_new < 0.0) abs_new = 0.0;\n"
    "    if (abs_new > 1.0) abs_new = 1.0;\n"
    "\n"
    "    // 6. Randbedingung (nur Phase verschieben, Amplitude bleibt erhalten)\n"
    "    if (is_boundary)\n"
    "    {\n"
    "        double dphi = phase_new - I_phase_prev[i];\n"
    "        double omega = dphi / DT;\n"
    "        if (omega < 1e-30) omega = 1e-30;\n"
    "        double phi_vac = phi_0 * pow(omega / omega_0, D - 3.0);\n"
    "        phase_new = phase_new + phi_vac;\n"
    "    }\n"
    "\n"
    "    // 7. Phase auf [-PI, PI] normieren\n"
