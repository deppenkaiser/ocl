...
protected const char* ocl_get_source_iwt_update(void)
{
    return
    "// ============================================================\n"
    "// IWT-Kernel – Kopplungen aus IWT-Parametern\n"
    "// ============================================================\n"
    "\n"
    "float iwt_calculate_charge(float I_i, __global float* nodes, __global uint* adjacency, uint i) {\n"
    "    float sum = 0.0f;\n"
    ...
    "}\n"
    "\n"
    "float iwt_calculate_mass(float I_i, __global float* nodes, __global uint* adjacency, uint i) {\n"
    "    float sum = 0.0f;\n"
    "    uint start = adjacency[i * 2];\n"
    "    uint end = adjacency[i * 2 + 1];\n"
    "    for (uint e = start; e < end; e++) {\n"
    "        uint j = adjacency[e];\n"
    "        float diff = I_i - nodes[j];\n"
    "        sum += diff * diff;\n"
    "    }\n"
    "    return sum;\n"
    "}\n"
    "\n"
    "float iwt_wed_force(float q_i, float q_j, float r, float D, float l0, float k, float norm) {\n"
    "    if (r < 1e-12f) return 0.0f;\n"
    "    return k * norm * q_i * q_j / pow(r, D - 1.0f);\n"
    "}\n"
    "\n"
    "float iwt_wg_force(float m_i, float m_j, float r, float D, float l0, float G, float norm) {\n"
    "    if (r < 1e-12f) return 0.0f;\n"
    "    return -G * norm * m_i * m_j / pow(r, D - 2.0f);\n"
    "}\n"
    "\n"
    "float iwt_q_force(float I_i, float I_j, float r, float D, float l0, float Q, float norm) {\n"
    "    if (r < 1e-12f) return 0.0f;\n"
    "    return Q * norm / pow(r, D - 2.0f);\n"
    "}\n"
    "\n"
    ...
    "\n"
    "    float I_i = nodes[i];\n"
    "    float q_i = iwt_calculate_charge(I_i, nodes, adjacency, i);\n"
    "    float m_i = iwt_calculate_mass(I_i, nodes, adjacency, i);\n"
    "\n"
    "    float I_new = I_i;\n"
    "    float norm = 1.0f / (float)(num_nodes * num_nodes);\n"
    "\n"
    "    for (uint j = 0; j < num_nodes; j++) {\n"
    "        if (i == j) continue;\n"
    "\n"
    "        float I_j = nodes[j];\n"
    "        float q_j = iwt_calculate_charge(I_j, nodes, adjacency, j);\n"
    "        float m_j = iwt_calculate_mass(I_j, nodes, adjacency, j);\n"
    "\n"
    "        float dist = (float)(j - i);\n"
    "        if (dist < 0.0f) dist = -dist;\n"
    "        float r = pow(dist, D - 2.0f) * l0;\n"
    "\n"
    "        float F_wed = iwt_wed_force(q_i, q_j, r, D, l0, k, norm);\n"
    "        float F_wg  = iwt_wg_force(m_i, m_j, r, D, l0, G, norm);\n"
    "        float F_q   = iwt_q_force(I_i, I_j, r, D, l0, Q, norm);\n"
    "\n"
    "        I_new += (F_wed + F_wg + F_q) * dt;\n"
    "    }\n"
    "\n"
    "    if (I_new > 10.0f) I_new = 10.0f;\n"
