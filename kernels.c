...
{
    return
    "double iwt_calculate_charge(double I_i, __global double* nodes, __global uint* adjacency, uint i) {\n"
    "    double sum = 0.0;\n"
    ...
    "}\n"
    "\n"
    "double iwt_calculate_mass(double I_i, __global double* nodes, __global uint* adjacency, uint i) {\n"
    "    double sum = 0.0;\n"
    "    uint start = adjacency[i * 2];\n"
    "    uint end = adjacency[i * 2 + 1];\n"
    "    for (uint e = start; e < end; e++) {\n"
    "        uint j = adjacency[e];\n"
    "        double diff = I_i - nodes[j];\n"
    "        sum += diff * diff;\n"
    "    }\n"
    "    return sum;\n"
    "}\n"
    "\n"
    "double iwt_wed_force(double q_i, double q_j, double r, double D, double l0, double k) {\n"
    "    if (r < 1e-12) return 0.0;\n"
    "    return k * q_i * q_j / pow(r, D - 1.0);\n"
    "}\n"
    "\n"
    "double iwt_wg_force(double m_i, double m_j, double r, double D, double l0, double G) {\n"
    "    if (r < 1e-12) return 0.0;\n"
    "    return -G * m_i * m_j / pow(r, D - 2.0);\n"
    "}\n"
    "\n"
    "double iwt_q_force(double I_i, double I_j, double r, double D, double l0, double Q) {\n"
    "    if (r < 1e-12) return 0.0;\n"
    "    return Q / pow(r, D - 2.0);\n"
    "}\n"
    "\n"
    ...
    "\n"
    "    double I_i = nodes[i];\n"
    "    double q_i = iwt_calculate_charge(I_i, nodes, adjacency, i);\n"
    "    double m_i = iwt_calculate_mass(I_i, nodes, adjacency, i);\n"
    "\n"
    "    double I_new = I_i;\n"
    "\n"
    ...
    "\n"
    "        double I_j = nodes[j];\n"
    "        double q_j = iwt_calculate_charge(I_j, nodes, adjacency, j);\n"
    "        double m_j = iwt_calculate_mass(I_j, nodes, adjacency, j);\n"
    "\n"
    "        double dist = (double)(j - i);\n"
    "        if (dist < 0.0) dist = -dist;\n"
    "        double r = pow(dist, D - 2.0) * l0;\n"
    "\n"
    "        double F_wed = iwt_wed_force(q_i, q_j, r, D, l0, k);\n"
    "        double F_wg  = iwt_wg_force(m_i, m_j, r, D, l0, G);\n"
    "        double F_q   = iwt_q_force(I_i, I_j, r, D, l0, Q);\n"
    "\n"
    "        double F_total = (F_wed + F_wg + F_q) * 1e45;\n"
    "\n"
    "        if (i == 0 && j < 4) {\n"
    "            flows[j] = F_total;\n"
    "        }\n"
