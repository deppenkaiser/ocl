...
protected const char* ocl_get_source_compute_flux(void)
{
    return
    ...
    "    {\n"
    "        if (i == j) continue;\n"
    "        uint idx = i * num_nodes + j;\n"
    "        J[idx] = K[idx] * (I[i] - I[j]);\n"
    "    }\n"
    "}\n";
}

protected const char* ocl_get_source_compute_q(void)
{
    return
    "__kernel void compute_q(\n"
    "    __global const double* J,\n"
    "    __global double* Q,\n"
    "    uint num_nodes\n"
    ")\n"
    "{\n"
    ...
    "    {\n"
    "        if (i == j) continue;\n"
    "        inflow += J[j * num_nodes + i];\n"
    "        outflow += J[i * num_nodes + j];\n"
    "    }\n"
    "\n"
    "    Q[i] = inflow - outflow;\n"
    "}\n";
...
protected const char* ocl_get_source_update_I(void)
{
    return
    "__kernel void update_I(\n"
    "    __global double* I,\n"
    "    __global const double* J,\n"
    "    double dt,\n"
    "    uint num_nodes\n"
...
protected const char* ocl_get_source_update_K(void)
{
    return
    "__kernel void update_K(\n"
    "    __global double* K,\n"
    "    __global const double* I,\n"
    "    double eta,\n"
    "    double lambda,\n"
    ...
    ")\n"
    "{\n"
    "    uint li = get_global_id(0);\n"
    "    uint lj = get_global_id(1);\n"
    "    if (li >= batch_i || lj >= batch_j) return;\n"
    "\n"
    "    uint i = offset_i + li;\n"
    "    uint j = offset_j + lj;\n"
    ...
    "    double diff = I[i] - I[j];\n"
    "\n"
    "    double delta = eta * (diff * diff - 2.0 * lambda * K[idx]) * dt;\n"
    "    K[idx] += delta;\n"
    "\n"
    "    if (K[idx] < 0.0) K[idx] = 0.0;\n"
    "}\n";
}
