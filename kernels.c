...
protected const char* ocl_get_source_iwt_flux(void)
{
    ...
    "    __global double* R_out,\n"
    "    __global double* T_out,\n"
    "    int N,\n"
    "    double DT,\n"
    "    double Z_0,\n"
    "    double alpha)\n"
    ...
    "\n"
    "    double Z_i = Z_0 * (1.0 + alpha * abs_i);\n"
    "\n"
    "    double sum = 0.0;\n"
    "    double R_sum = 0.0;\n"
    "    double T_sum = 0.0;\n"
    ...
    "\n"
    "        double Kij = K[i * N + j];\n"
    "        double Z_j = Z_0 * (1.0 + alpha * abs_j);\n"
    "\n"
    "        double R = (Z_j - Z_i) / (Z_j + Z_i + 1e-30);\n"
    "        double T = 1.0 - R;\n"
    "\n"
    "        R_sum += R;\n"
    "        T_sum += T;\n"
    ...
    "        sum += Kij * (rho_i - rho_j) * T;\n"
    "    }\n"
    "\n"
    "    // Begrenzung\n"
    "    double max_out = rho_i / DT;\n"
    "    double max_in = (1.0 - rho_i) / DT;\n"
    "    if (sum > max_out) sum = max_out;\n"
    "    if (sum < -max_in) sum = -max_in;\n"
    "\n"
    "    sumJ[i] = sum;\n"
    "    R_out[i] = R_sum;\n"
    "    T_out[i] = T_sum;\n"
...
protected const char* ocl_get_source_iwt_update_info(void)
{
    ...
    "    __global const double* I_prev_real,\n"
    "    __global const double* I_prev_imag,\n"
    ...
    "    if (i >= N) return;\n"
    "\n"
    "    double Re_old = I_real[i];\n"
    "    double Im_old = I_imag[i];\n"
    "    double abs_old = sqrt(Re_old * Re_old + Im_old * Im_old + 1e-30);\n"
    "    double rho_old = abs_old * abs_old;\n"
    "\n"
    "    // Fluss aus dem Kernel: sumJ[i] = ∇·J (für rho = |I|²)\n"
    "    double flow = sumJ[i];\n"
    "\n"
    ...
    "\n"
    "    // Fluktuationen wirken auf Real- und Imaginärteil (Anhang O & P)\n"
    ...
    "    if (phase_new < -PI) phase_new = phase_new + twoPI;\n"
    "\n"
    "    // Aus der neuen Dichte rho_new und der Phase phase_new\n"
    "    // die neuen Real- und Imaginärteile berechnen:\n"
    "    // Re = sqrt(rho) * cos(phi), Im = sqrt(rho) * sin(phi)\n"
