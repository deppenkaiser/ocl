...
protected const char* ocl_get_source_iwt_q(void)
{
    ...
    "\n"
    "    // 2. Diskreter Laplace von sqrt(rho) (ungewichtet)\n"
    "    double laplace = 0.0;\n"
    "    for (int j = 0; j < N; j++)\n"
    "    {\n"
    "        if (i == j) continue;\n"
...
protected const char* ocl_get_source_iwt_update_info(void)
{
    ...
    "    if (i >= N) return;\n"
    "\n"
    "    // 1. Entropie-Term: -gamma * ln(I / I0)\n"
    "    double entropy = GAMMA_ENTROPY * log(I[i] / I0);\n"
    "\n"
    "    // 2. Q als Potential: -ETA_Q_POTENTIAL * Q[i]\n"
    "    double potential = ETA_Q_POTENTIAL * Q[i];\n"
    "\n"
    "    // 3. Materieentstehung durch Q: +ETA_SOURCE * |grad Q|^2\n"
    "    //    Gradient von Q (diskrete Ableitung)\n"
    "    double grad_Q_sq = 0.0;\n"
    "    for (int j = 0; j < N; j++)\n"
    "    {\n"
    "        if (i == j) continue;\n"
    "        double dQ = Q[j] - Q[i];\n"
    ...
    "    }\n"
    "\n"
    "    // 4. Euler-Lagrange mit allen Termen\n"
    "    double I_new = 2.0 * I[i] - I_prev[i]\n"
    "        + DT * DT * sumJ[i]\n"
    "        - DT * DT * entropy\n"
    "        - DT * DT * potential\n"
    "        + DT * DT * ETA_SOURCE * grad_Q_sq;\n"
    "\n"
    "    I[i] = I_new;\n"
    "}\n";
}
...
protected const char* ocl_get_source_iwt_update_coupling(void)
{
    ...
    "\n"
    "        double diff = Ii - Ij;\n"
    "        if (diff > I_max - I_min) diff = I_max - I_min;\n"
    "        if (diff < -(I_max - I_min)) diff = -(I_max - I_min);\n"
    "\n"
    "        double I_mid = (Ii + Ij) * 0.5;\n"
    "        if (I_mid < 1e-30) I_mid = 1e-30;\n"
    "\n"
    "        double laplace_I = Ii - 2.0 * I_mid + Ij;\n"
    "\n"
    "        double Q_diff = Qi - Qj;\n"
    "\n"
    "        double Kij = K[idx * N + j];\n"
    "\n"
    "        // Lokale Dichte für fraktalen Term\n"
    "        double rho_local = (Ii + Ij) * 0.5;\n"
    "\n"
    "        // Vollständige K-Dynamik mit lokalem fraktalen Term\n"
    "        double K_new = Kij + DT * (\n"
    "            ETA * (diff * diff)\n"
    "            - LAMBDA * laplace_I / I_mid\n"
    "            - MU * Q_diff\n"
    "            + GAMMA * Kij * log(1.0 + rho_local * L * L)\n"
    "        );\n"
    "\n"
    "        K[idx * N + j] = K_new;\n"
    "        K[j * N + idx] = K_new;\n"
