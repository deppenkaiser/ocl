...
protected const char* ocl_get_source_iwt_q(void)
{
    ...
    "\n"
    "    // 2. Diskreter Laplace von sqrt(rho) mit Kopplungsmatrix\n"
    "    double laplace = 0.0;\n"
    "    for (int j = 0; j < N; j++)\n"
    "    {\n"
    "        if (i == j) continue;\n"
...
protected const char* ocl_get_source_iwt_update_coupling(void)
{
    ...
    "    double Qi = Q[idx];\n"
    "\n"
    "    // g(I) - Potentialfunktion mit Nullstellen bei Fixpunkten\n"
    "    double gi = (Ii - 0.01)*(Ii - 0.01)\n"
    "              * (Ii - 0.25)*(Ii - 0.25)\n"
    "              * (Ii - 1.0)*(Ii - 1.0)\n"
    "              * ((Ii - 0.05)*(Ii - 0.05) + 0.01)\n"
    "              * ((Ii - 0.06)*(Ii - 0.06) + 0.01);\n"
    "\n"
    "    for (int j = 0; j < N; j++)\n"
    "    {\n"
    "        double Ij = I[j];\n"
    ...
    "\n"
    "        // g(I_j)\n"
    "        double gj = (Ij - 0.01)*(Ij - 0.01)\n"
    "                  * (Ij - 0.25)*(Ij - 0.25)\n"
    "                  * (Ij - 1.0)*(Ij - 1.0)\n"
    "                  * ((Ij - 0.05)*(Ij - 0.05) + 0.01)\n"
    "                  * ((Ij - 0.06)*(Ij - 0.06) + 0.01);\n"
    "\n"
    "        double Kij = K[idx * N + j];\n"
    "\n"
    "        // Zerfall nur zwischen Fixpunkten (g>0), nicht an Fixpunkten (g=0)\n"
    "        double K_new = Kij + ETA * ((diff * diff) - 2.0 * LAMBDA * gi * gj * Kij - MU * Q_diff) * DT;\n"
    "\n"
    "        K[idx * N + j] = K_new;\n"
    "        K[j * N + idx] = K_new;\n"
    "    }\n"
